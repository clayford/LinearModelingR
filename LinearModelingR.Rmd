...
- each additional finished square foot adds about $272 to price
- each additional bedroom drops the price by $34734 (?)
...
When dealing with a response that is strictly positive and very skew (like dollar amounts), it is common to transform the response to a different scale. A common transformation is a _log transformation_. When we log transform `totalvalue`, the distribution looks a little more symmetric, though it's important to note that is not an assumption of linear modeling!
...
For example, each additional bedroom (assuming all else held constant) increases expected total price by about 4.4%. Multiplying by 1.0444 is equivalent to adding 4.4%.
...
Homes 11, 19, 1089 and 2746 seem to stand out. Let's have a look.
...
Homes 11 and 19 are very low in totalvalue and the model way overpredicts their values. Home 1089 has a relatively small totalvalue with 23 acres of lotsize and over 4700 finsqft, but the model way overpredicts its totalvalue as well. Home 2746 is on 140 acres and valued at over 6 million dollars, but has an estimated totalvalue of just over 2 million.
