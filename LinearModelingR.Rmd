...
Instead of using theory and formulas, let's learn linear modeling using simulated data. 
...
We can visualize this Normal distribution in R using the `curve()` function. The `dnorm()` function is the formula that produces the normal bell curve. Below we draw it with x values ranging from -30 to 30 (ie, 3 standard deviations).

```{r}
curve(dnorm(x, mean = 0, sd = 10), from = -10*3, to = 10*3)
```

The `rnorm()` function allows us to draw random values from a Normal distribution.
...
1. the formula is a _weighted sum_ of predictors (eg, y = 10 + 5x); the weights (or coefficients) are a _linear function_ of the predictors. In other words, the weights multiply the predictor (5x), they don't divide or exponentiate (x/5 or x^5)
...
points(d$x, d$y2, col = "red") # model-simulated data
```
...
d$y2 <- 11.135 + 5.042*d$x + rnorm(25, 0, 9.7)
lines(density(d$y2), col = "red")  # model-simulated data
```
...
- *fp*: indicator if house has fireplace (No, Yes)
...
```
totalprice = -116423.2343 + 372.8104*finsqft + -34734.0164*bedroom +
               3660.7231*lotsize`
```

Some naive interpretation:

- each additional finished square foot adds about $372 to price
- each additional bedroom drops the price by $34,734 (?)
- each additional lot size acre adds about $3660 to price

Each of these interpretations assumes _all other variables are held constant_! So adding a bedroom to a house, without increasing the lot size or finished square feet of the house, is estimated to drop the value of a home.  
...
```{r}
sim_values <- -116423.2343 + 372.8104*homes$finsqft + 
  -34734.0164*homes$bedroom + 3660.7231*homes$lotsize + 
  rnorm(3288, mean = 0, sd = 265800)
```
...
2. Normal Q-Q: for checking normality of residuals; points should lie close to diagonal line; if not, evidence that noise is not drawn from N(0, SD). _This assumption is only critical if you're planning to use your model to predict individual values (instead of means) and calculate a confidence interval._ 
...
By default the 3 "most extreme" points are labeled by row number. 3285 appears in all four plots. It's a really big expensive home.
...
When dealing with a response that is strictly positive and very skew (like dollar amounts), it is common to transform the response to a different scale. A common transformation is a _log transformation_. When we log transform `totalvalue`, the distribution looks a little more symmetric, though it's important to note that is _not an assumption of linear modeling_!
...
The diagnostic plots look somewhat better. 

```{r}
plot(m2)
```

Homes 700, 1131, and 469 seem to be influencing our model.

```{r}
homes[c(700, 1131, 469), c("totalvalue", "finsqft", "bedroom", "lotsize")]
```
...
These are approximate _proportional_ differences. To get percentages, multiply by 100.
...
- each additional finished square foot increases expected mean total value by 0.04%. Or multiply by 100 to say each additional 100 finished square feet increases expected mean total value by 4%.
- each additional bedroom increases expected mean total value by about 4.4%
- each additional lot size acre increases expected mean total value by about 0.31%
...
- Each additional finished square foot increases expected total price by about 0.04%. (Multiplying by 1.0004 is equivalent to adding .04%.) Adding 100 square feet increases expected total price by about 4%. 
- Each additional bedroom (assuming all else held constant) increases expected total price by about 4.4%. (Multiplying by 1.0444 is equivalent to adding 4.4%.)
- Each additional acre increases expected value by about 0.31%.  (Multiplying by 1.0031 is equivalent to adding 0.31%.)
...
- degrees of freedom: sample size - number of coefficients (3284 - 4)
...
Let's add `hsdistrict` to the model we just fit. Does being in a particular high school district affect expected total value of a home? Here's a quick tally:
...
Let's look at the default contrast, called a _treatment contrast_. To do this we use the `contrasts()` function. 
...
It appears that the value of a home in Western Albemarle will be about 10% higher than an equivalent home in Albemarle. Likewise it appears that the value of a home in the Monticello district will be about 3% less than an equivalent home in the Albemarle district. 
...
`ggpredict` used our model to make `totalvalue` predictions for various values of `fullbath` in the three school districts, holding finsqft at 1936 (the median of finsqft). 
...
Homes 469, 700 and 2746 seem to stand out. Let's have a look.

```{r}
h <- c(469, 700, 2746)
fit <- exp(fitted(nlm3)[h]) # get fitted values
cbind(homes[h,c("totalvalue","finsqft", "lotsize", "hsdistrict")], fit)
```
...
Home 469 is way overpredicted to be 5.3 million when it's only 833,000. Likewise home 700 is overpredicted to be 11 million when it's "only" 3 million. Home 2476 is way underpredicted at 2 million when it's valued at over 6 million

The model is adequate for most homes, but not all. 
...
- Faraway, J. (2025). _Linear Models in R_. (3rd ed.). London: Chapman & Hall.
- Fox, J. (2019). _A R Companion to Applied Regression_ (3rd ed.). London: Sage.
...
Ridge regression is another method sometimes employed to deal with collinearity.
...
0.28 is close to 0, so it makes sense to proceed with a log transformation. That greatly simplifies interpretation.
