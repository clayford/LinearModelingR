...
- The `visreg` package provides a nice function called `visreg` to easily plot regression lines with confidence intervals.
...
- To get an idea of how they interact we can create an interaction plot or visualize the model using a package such as `visreg`.
...
* Character variables are automatically converted to factors on import if using base R `read.` functions.
...
- `effects`: visualizing model effects; similar to `visreg` but with a few more features
